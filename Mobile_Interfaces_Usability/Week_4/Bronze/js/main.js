...
// Create select field element and populate with options.
    function getTrailRating() {
    	...
    	makeSelect.setAttribute("id", "physicalSelect"); // setting the id to physicalSelect
    	for (var i=0, j=trailRatings.length; i<j; i++) {  //looping through physicalSelect
    ...
    function getTerrainRating() {
    	...
    	var selectLi = re('selectOptions');
    	var makeSelect = document.createElement('select'); // creates the element
    	makeSelect.setAttribute("id", "terrainSelect"); // setting the id to terrainSelect
    	for (var i=0, j=terrainRatings.length; i<j; i++) {  //looping through terrainSelect
    	    var makeOption = document.createElement('option'); // create an option for each of the strings in the array
	...
	//Search function
	/*function getSearch() {
		var phyCategory = re('physicalSelect').value;
		var terCategory = re('terrainSelect').value;
		var term = re('search').value;
		
		//Search by Physical Category only
		if (phyCategory != "Physical Difficulty" && terCategory == "Terrain Conditions" && term == "") {
			for (var i=0, l=localStorage.length; i<l; i++) {
				var key = localStorage.key(i);// loop through local storage &  find the key
				var value = localStorage.getItem(key); // return the value
				var trailObject = JSON.parse(value); //turning string back into an object(.parse)
				if (phyCategory === ride.physical[1]) {
					for(n in trailObject) {
						console.log(trailObject[n][0]+": "+trailObject[n][1]);// Properties are in arrays
					}// end 2nd conditional
				}// end second conditional
			}// end for loop	
		} // end first conditional
		// Search by Terrain condition only
		/*if (terCategory != "Terrain Conditions" && phyCategory == "Physical Difficulty" && term == "") {
			ride.terrain
		}
		...
		// Search by Physical AND term
		if (phyCategory != "Physical Difficulty" && terCategory == "Terrain Conditions" && term != "") {
			
		}
		
		//search by Terrain and term
		if (terCategory != "Terrain Conditions" && phyCategory == "Physical Difficulty" && term != "") {
			
		}
		
		// Search by Physical and Terrain
		if (terCategory != "Terrain Conditions" && phyCategory != "Physical Difficulty" && term == "") {
			
		}
		
		//Search by Physical, Terrain, and term
		if (terCategory != "Terrain Conditions" && phyCategory != "Physical Difficulty" & term !="") {
